...
%token TDEF EXTERN STATIC AUTO REGISTER INLINE RESTRICT
%token CHAR SHORT INT LONG SIGNED UNSIGNED FLOAT DOUBLE CONST VOLATILE VOID
%token _BOOL _COMPLEX _IMAGINARY
%token STRUCT UNION ENUM
%token BREAK CASE CONT DEFAULT DO IF ELSE FOR GOTO WHILE SWITCH SIZEOF
%token RETURN

%token ELLIPSIS RIGHT_ASSGN LEFT_ASSGN ADD_ASSGN SUB_ASSGN MUL_ASSGN
%token DIV_ASSGN MOD_ASSGN AND_ASSGN XOR_ASSGN OR_ASSGN RIGHT_OP LEFT_OP
%token INC_OP DEC_OP PTR_OP AND_OP OR_OP LE_OP GE_OP EQ_OP NE_OP

%token IDENTIFIER STR_LITERAL PUNCTUATORS COMMENT
%token INT_CONST FLOAT_CONST ENU_CONST CHAR_CONST
